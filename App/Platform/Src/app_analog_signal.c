...
ATTR_PLACE_AT_FAST_RAM_INIT static const adc_channel_t s_item_to_channel[APP_ANALOG_SIGNAL_ITEM_COUNT] = {
    [APP_ANALOG_SIGNAL_ITEM_VCAP] = APP_ADC_CH_VLINK,
    [APP_ANALOG_SIGNAL_ITEM_VOUT] = APP_ADC_CH_VIN,
...
ATTR_PLACE_AT_FAST_RAM_INIT const app_analog_signal_item_t s_channel_to_item[ADC_CH_COUNT] = {
    [APP_ADC_CH_VLINK] = APP_ANALOG_SIGNAL_ITEM_VCAP,
    [APP_ADC_CH_VIN] = APP_ANALOG_SIGNAL_ITEM_VOUT,
...
static const app_adc_calibration_t s_default_calibration[ADC_CH_COUNT] = {
    ...
    [ADC_CH_I_IN] = {
        ...
        .sense_offset_mv = 0.0f,
        /*
         * PCB layout reverses PB13/I_IN current-sense polarity relative to the
         * software convention.  Use a negative gain so physical I_IN becomes
         * (bias_mv - sense_mv) * A/V instead of (sense_mv - bias_mv) * A/V.
         */
...
static const app_analog_signal_filter_cfg_t s_default_filter_cfg[ADC_CH_COUNT] = {
    [ADC_CH_VCAP] = {.type = APP_ANALOG_SIGNAL_FILTER_LPF, .cfg.lpf = {.cutoff_hz = 40000.0f, .sample_rate_hz = 200000.0f}},
    [ADC_CH_VOUT] = {.type = APP_ANALOG_SIGNAL_FILTER_MA, .cfg.ma = {.window_size = 4U}},
    [ADC_CH_I_IN] = {.type = APP_ANALOG_SIGNAL_FILTER_LPF, .cfg.lpf = {.cutoff_hz = 20.0f, .sample_rate_hz = 25000.0f}},
    [ADC_CH_I_L]  = {.type = APP_ANALOG_SIGNAL_FILTER_LPF, .cfg.lpf = {.cutoff_hz = 20000.0f, .sample_rate_hz = 200000.0f}},
};
...
static void app_analog_signal_update_fast_calibration(adc_channel_t ch, const app_adc_calibration_t *cal)
{
...
static void app_analog_signal_filter_init_one(app_analog_signal_item_t item)
{
    ...
    if (item == APP_ANALOG_SIGNAL_ITEM_VCAP || item == APP_ANALOG_SIGNAL_ITEM_I_IN) {
        algo_ma_cfg_t ma_cfg = {
            .window_size = (item == APP_ANALOG_SIGNAL_ITEM_I_IN) ? 8U : 4U,
            .buffer = s_ma_filter_buffers[item],
        };
        (void)s_ma_filters[item].init(&s_ma_filters[item], &ma_cfg);
    }
...
static float app_analog_signal_filter_step(app_analog_signal_item_t item, float value)
{
    ...
    case APP_ANALOG_SIGNAL_FILTER_LPF:
        return s_lpf_filters[item]._inited ? algo_lpf_step_fast(&s_lpf_filters[item], value) : value;
    default:
        return value;
    }
}

static float *snapshot_field_ptr(app_analog_signal_measurements_t *m, app_analog_signal_item_t item)
{
    switch (item) {
...
void app_analog_signal_snapshot_refresh_raw(void)
{
    for (adc_channel_t ch = ADC_CH_VCAP; ch < ADC_CH_COUNT; ch++) {
        ...
        if (app_adc_get_pmt_raw(ch, &raw) == 0) {
            ...
            float *raw_field = snapshot_field_ptr(&g_analog_signal_snapshot.raw, s_channel_to_item[ch]);
            if (raw_field != NULL) {
                *raw_field = physical;
            }
        }
    ...
    app_analog_signal_measurements_t *measurements, app_analog_signal_value_mode_t mode)
{
    ...
    *measurements = (app_analog_signal_measurements_t){0};
    ...
    if (app_analog_signal_read_item(APP_ANALOG_SIGNAL_ITEM_VCAP, mode, &measurements->vcap_v) != 0) {
        return -1;
    }
    if (app_analog_signal_read_item(APP_ANALOG_SIGNAL_ITEM_VOUT, mode, &measurements->vout_v) != 0) {
        return -1;
    }
